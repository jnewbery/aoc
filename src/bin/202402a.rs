...
fn is_safe(vec: Vec<i32>) -> bool {
    vec.windows(2)
        ...
        .fold(Ok(None), |is_inc, diff| match is_inc {
            ...
            Ok(None) if [1,2].contains(&diff) => Ok(Some(true)), // Initialize to increasing
            Ok(None) if [-1,-2].contains(&diff) => Ok(Some(false)), // Initialize to decreasing

            Ok(Some(true)) if [1,2].contains(&diff) => Ok(Some(true)), // Continue increasing
            Ok(Some(false)) if [-1,-2].contains(&diff) => Ok(Some(false)), // Continue decreasing
