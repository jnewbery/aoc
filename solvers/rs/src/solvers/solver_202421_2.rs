...
fn type_pair(pair: (Point, Point), numeric: bool) -> HashSet<String> {
    ...
    let ver_steps: Vec<char> = if from_point.y < to_point.y {
        (from_point.y..to_point.y).map(|_| '^').collect()
    } else if from_point.y > to_point.y {
        (to_point.y..from_point.y).map(|_| 'v').collect()
    } else {vec![]};

    let hor_steps: Vec<char> = if from_point.x < to_point.x {
        (from_point.x..to_point.x).map(|_| '>').collect()
    } else if from_point.x > to_point.x {
        (to_point.x..from_point.x).map(|_| '<').collect()
    } else {vec![]};

    // paths is all vertical steps followed by all horizontal steps, and all horizontal steps
    // followed by all vertical steps
    let mut paths: HashSet<Vec<char>> = HashSet::new();
    paths.insert(hor_steps.iter().chain(ver_steps.iter()).cloned().collect());
    paths.insert(ver_steps.iter().chain(hor_steps.iter()).cloned().collect());
...
fn type_code(
    ...
    cache: &mut HashMap<(String, u64, bool), u64>) -> u64
{
...
fn solve_line(
    ...
    cache: &mut HashMap<(String, u64, bool), u64>) -> u64
{
    let numeric_part = line.chars().filter(|c| c.is_numeric()).collect::<String>().parse::<u64>().unwrap();
    let path_len = type_code(line, 26, true, directional_keypad, numeric_keypad, cache);
...
pub fn solve(input: &str) -> String {
    ...
    let mut score: u64 = 0;
    let mut cache: HashMap<(String, u64, bool), u64> = HashMap::new();
    for line in lines {
